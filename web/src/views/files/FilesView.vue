...
:current-dir="currentDir" :show-hidden="filter.showHidden" :can-paste="canPaste()"
        ...
        @toggle-show-hidden="onToggleShowHidden" />
    </div>
  </div>
...
import { useCreateDir, useRename, useDownload, useVolumes, useView, useCopyPaste, useSearch } from '@/hooks/files'
...
import { initMutation, setTempValueGQL, addFavoriteFolderGQL, trashFilesGQL, restoreFilesGQL } from '@/lib/api/mutation'
import type { ISource } from '@/components/lightbox/types'
import type { IFileRenamedEvent, IFileFilter, IBreadcrumbItem, IMediaItemsActionedEvent } from '@/lib/interfaces'
...
// rootDir derived from query.q -> root_path
...
const currentDir = computed(() => {
  if (!filter.rootPath) return ''
  if (!filter.relativePath) return filter.rootPath
  const rel = filter.relativePath.startsWith('/') ? filter.relativePath.substring(1) : filter.relativePath
  return `${filter.rootPath}/${rel}`.replace(/\/+/, '/').replace(/\/+/, '/')
})

const breadcrumbCurrentDir = computed(() => {
  const v = currentDir.value
  return v.replace(/\/+$/, '')
})
...
function getPageTitle() {
  ...
  // Fallbacks
  if (rootDir.value) {
    return getFileName(rootDir.value) || t('internal_storage')
  }
...
function navigateToDir(dir: string) {
  ...
  if (dir.startsWith(rootDir.value)) {
    // Normal browsing uses (root_path + relative_path).
    const rel = dir.substring(rootDir.value.length)
    filter.relativePath = rel.startsWith('/') ? rel.substring(1) : rel
  } else {
...
function onToggleShowHidden() {
  filter.showHidden = !filter.showHidden
  mainStore.fileShowHidden = filter.showHidden
  const nextQ = buildQ(filter)
  replacePath(mainStore, getUrl(nextQ))
}
...
function deleteItem(item: IFile) {
  trashFilesMutation({ paths: [item.path] }).then(() => {
    ...
    // Ensure listing reflects actual filesystem state after trashing.
    fetch()
  })
}
...
function addToFavoritesClick(item: IFile) {
  ...
  // If item is outside current root, treat item.path as new root
...
onActivated(() => {
  ...
  filter.showHidden = mainStore.fileShowHidden
  fetch()
