...
v-if="items.length > 0" class="scroller main-list" :data-key="'id'" :data-sources="items"
    :estimate-size="isPhone ? 120 : 80">
    <template #item="{ index, item }">
      <FileRecentItem
:key="item.id" :item="item" :index="index" :selected-ids="selectedIds"
        ...
        :on-image-error="onImageError" :on-extension-image-error="onExtensionImageError" :download-file="downloadFile"
...
import type { IFileDeletedEvent, IFileRenamedEvent, IMediaItemsActionedEvent } from '@/lib/interfaces'
...
const onDeleted = (paths: string[]) => {
  paths.forEach((path) => {
    remove(items.value, (it: IFile) => it.path === path)
  })
  clearSelection()
}

const fileDeletedHanlder = (event: IFileDeletedEvent) => {
  onDeleted(event.paths)
}
