<template>
  ...
  <v-icon-button v-tooltip="$t('search_filter_show_hidden')" toggle @click="toggleShowHidden">
    <i-material-symbols:visibility-rounded v-if="showHidden" />
    <i-material-symbols:visibility-off-outline-rounded v-else />
  </v-icon-button>
...
v-for="item in sortItems" :key="item.value" class="dropdown-item"
      :class="{ 'selected': item.value === fileSortBy }" @click="sort(item.value); sortMenuVisible = false">
      {{ $t(item.label) }}
    </div>
  </v-dropdown>
</template>
