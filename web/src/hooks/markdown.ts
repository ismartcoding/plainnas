...
export const useMarkdown = (app: Ref<{ dataDir: string }>, urlTokenKey: Ref<sjcl.BitArray | null>) => {
  ...
  const replace = (link: string) => {
    if (link.startsWith('app://')) {
      const { dataDir } = app.value
      return getFileUrlByPath(urlTokenKey.value, dataDir + '/' + link.replace('app://', ''))
    }
    return link
  }
